...
def generate_coreset(vae_model, task_id, coreset_size):
	pass

def train_vae(vae_model, vae_optimizer, train_loader, coreset_loader, args, current_task_id):
	pass
...
def run(args):
	...
	for current_task_id in range(1, args.tasks+1):
		...
		for epoch in range(1, args.epochs_per_task+1):
			...
			train_vae(vae_model, vae_optimizer, train_loader, coreset_loader, criterion, bias, lmd, gma, bta, args, current_task_id, writer)
			...
			for prev_task_id in range(1, current_task_id+1):
				...
				if epoch == args.epochs_per_task:
					...
					coreset_dataset = generate_coreset(vae_model, task_id = prev_task_id-1, coreset_size = args.core_size)
