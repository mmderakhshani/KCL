...
# def train_embedding(net, optimizer, loader, coreset_loader, task_embedding, task_embedding_label, criterion, bias, lmd, gma, bta, args, task_id=None, writer=None):
# 	"""
# 	Train the model for a single epoch
...
# 	coreset_input  = coreset_input.to(DEVICE)
# 	# coreset_target = torch.nn.functional.one_hot(coreset_target, num_classes=10 if "mnist" in args.dataset else 100)
# 	coreset_target = coreset_target.to(DEVICE).long()
	
# 	task_embedding_optimizer = torch.optim.Adam([task_embedding], lr=1e-3)
...
# 		offset1 = int((task_id-1) * 5)
# 		offset2 = int(task_id * 5)
		
# 		if task_id:
# 			features_coreset, r_mu, r_log_var  = net(coreset_input, task_id, post = False, predefined=True)
# 			task_embd_features_coreset, task_embd_mu, task_embd_logvar = net(task_embedding[offset1:offset2], task_id, post = True)
# 		else:
# 			features_coreset, r_mu, r_log_var  = net(coreset_input, post = False, predefined=True)
# 			task_embd_features_coreset, task_embd_mu, task_embd_logvar = net(task_embedding[offset1:offset2], post = True)

# 		# compute_basesx
# 		rs = sample(task_embd_mu, task_embd_logvar, args.d_rn_f, DEVICE).squeeze()
# 		# Here we map the coreset data into random fourier space. 
# 		x_supp_phi_t = rand_features(rs, torch.transpose(task_embd_features_coreset, 1, 0), bias)  # (d_rn_f , number of coreset samples)
# 		# in the random fourier space we compute the the kernel matrix, the dot product between mapped samples
# 		support_kernel = dotp_kernel(torch.transpose(x_supp_phi_t, 1, 0), x_supp_phi_t)  # (number of coreset samples , number of coreset samples)
...
# 		# closed-form solution with trainable lmd
# 		# (number of coreset samples, number of classes)
# 		alpha = torch.matmul(torch.inverse(support_kernel + (torch.abs(lmd[task_id-1]) + 0.01) * torch.eye(support_kernel.shape[0]).to(DEVICE)), task_embedding_label[offset1:offset2]) 
		
# 		# Here we map the train data into random fourier space. 
# 		x_que_phi_t = rand_features(rs, torch.transpose(features_coreset, 1, 0), bias)
...
# 		loss = criterion(pred, coreset_target) + args.tau * kl_loss
# 		task_loss.update(loss.item())

# 		acc = (pred.argmax(dim=-1) == coreset_target).float().mean()
# 		task_acc.update(acc.item())
...
# 		if (i+1) % 10 == 0:
# 			grid = make_grid(task_embedding, nrow=5)
# 			writer.add_image('images', grid, vis_idx)
# 			# pdb.set_trace()
# 			# for i in range(len(task_embedding)):
# 			# 	writer.add_histogram("Histogram", task_embedding[i].data.cpu().numpy(), vis_idx)
# 			vis_idx = vis_idx + 1
...
# 			features_train,   p_mu, p_log_var = net(data, task_id, post = False, predefined=True)
# 			features_coreset, r_mu, r_log_var  = net(coreset_input, task_id, post = True)
# 			task_embd_features_coreset, task_embd_mu, task_embd_logvar = net(task_embedding[task_id-1], task_id, post = True)
# 		else:
# 			features_train,   p_mu, p_log_var = net(data,          post = False, predefined=True)
# 			features_coreset, r_mu, r_log_var  = net(coreset_input, post = True)
# 			task_embd_features_coreset, task_embd_mu, task_embd_logvar = net(task_embedding[task_id-1], post = True)
...
# 		# Forward task embedding through posterior distribution
# 		# Compute random basis over task embedding distribution
# 		# Compute random fourier features
# 		# Compute Estimated Kernel
# 		# Compute quadratic loss function such as mse
# 		# pdb.set_trace()
# 		task_embd_rs = sample(task_embd_mu, task_embd_logvar, args.d_rn_f, DEVICE).squeeze()
# 		task_embd_x_supp_phi_t = rand_features(task_embd_rs, torch.transpose(task_embd_features_coreset, 1, 0), bias)  # (d_rn_f , number of coreset samples)
# 		task_embd_support_kernel = dotp_kernel(torch.transpose(task_embd_x_supp_phi_t, 1, 0), task_embd_x_supp_phi_t)  # (number of coreset samples , number of coreset samples)
...
# 		# Here we map the train data into random fourier space. 
# 		x_que_phi_t           = rand_features(rs,           torch.transpose(features_train, 1, 0), bias)
# 		task_embd_x_que_phi_t = rand_features(task_embd_rs, torch.transpose(features_train, 1, 0), bias)

# 		# Compute cross kernel
# 		cross_kernel           = dotp_kernel(torch.transpose(x_supp_phi_t, 1, 0), x_que_phi_t) # (number of coreset samples, number of train samples)
# 		task_embd_cross_kernel = dotp_kernel(torch.transpose(task_embd_x_supp_phi_t, 1, 0), task_embd_x_que_phi_t) # (number of coreset samples, number of train samples)
...
# 		loss = loss_fn_kd(task_embd_pred, target, pred)
# 		task_loss.update(loss.item())
