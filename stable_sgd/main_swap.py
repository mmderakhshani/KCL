...
def train_single_epoch(net, optimizer, loader, coreset_loader, criterion, bias, lmd, gma, bta, args, task_id=None):
	...
	for _, (data, target) in enumerate(loader):
		...
		target_one_hot = torch.nn.functional.one_hot(target, num_classes=10 if "mnist" in args.dataset else 100).float()
		...
		for index in range(len(data)):
			...
			if task_id:
				features_train,   post_mu, post_log_var    = net(data, task_id, post = True)
				features_coreset, prior_mu, prior_log_var  = net(coreset_input, task_id, post = False)
			else:
				features_train,   post_mu, post_log_var    = net(data, post = True, predefined=True)
				features_coreset, prior_mu, prior_log_var  = net(coreset_input, post = False)


			selected_index  = torch.tensor([i for i in range(len(data)) if index != i]).to(DEVICE)
			selected_data   = torch.index_select(features_train, dim=0, index=selected_index)
			selected_target = torch.index_select(target_one_hot, dim=0, index=selected_index)
			
			leave_one_out_index   = torch.tensor([index]).to(DEVICE)
			leave_one_out_data    = torch.index_select(features_train, dim=0, index=leave_one_out_index)
			leave_one_out_target  = torch.index_select(target_one_hot, dim=0, index=leave_one_out_index)
		
		

			# compute_bases
			rs = sample(post_mu, post_log_var, args.d_rn_f, DEVICE).squeeze()

			# Here we map the coreset data into random fourier space. 
			x_supp_phi_t = rand_features(rs, torch.transpose(selected_data, 1, 0), bias)  # (d_rn_f , number of coreset samples)
			...
			alpha = torch.matmul(torch.inverse(support_kernel + (torch.abs(lmd[task_id-1]) + 0.01) * torch.eye(support_kernel.shape[0]).to(DEVICE)), selected_target) 
		
			# Here we map the train data into random fourier space. 
			x_que_phi_t = rand_features(rs, torch.transpose(leave_one_out_data, 1, 0), bias)
			...
			kl_loss = kl_div(post_mu, post_log_var, prior_mu, prior_log_var)
			loss = criterion(pred, target[index][None]) + args.tau * kl_loss
			acc = (pred.argmax(dim=-1) == target[index]).float().mean()

			total_losss = total_losss + loss
			total_acc   = total_acc   + acc


		task_loss.update(total_losss.item()/len(data))
		task_acc.update(total_acc.item()/len(data))

		end_time = benchmarker.time()
		time_complexity_history.update((end_time - start_time)/batch_size)

		total_losss.backward()
		optimizer.step()
...
def eval_single_epoch(net, loader, coreset_loader, criterion, bias, lmd, gma, bta, args, task_id=None):
	...
	with torch.no_grad():
		for data, target in loader:
			...
			target = target.to(DEVICE)
			if task_id:
				features_test,   p_mu, p_log_var = net(data, task_id, post = True)
				features_coreset, r_mu, r_log_var  = net(coreset_input, task_id, post = False)
			else:
				features_test,   p_mu, p_log_var = net(data,          post = True)
				features_coreset, r_mu, r_log_var  = net(coreset_input, post = False)
